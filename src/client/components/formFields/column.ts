...
import DataTableRowActions from '@/client/components/formFields/RowActions.vue'
import { Checkbox } from '@/client/components/ui/checkbox'
import type { FormField } from '~/client/api'

export const columns = (t:any):ColumnDef<FormField>[] => ([
  // {
  //   id: 'select',
  ...
  {
    accessorKey: 'order',
    header: ({ column }) => h(DataTableColumnHeader, { column, title: t('formFields.order') }),
  ...
  {
    accessorKey: 'name',
    header: ({ column }) => h(DataTableColumnHeader, { column, title: t('formFields.name') }),
  ...
  {
    accessorKey: 'type',
    header: ({ column }) => h(DataTableColumnHeader, { column, title: t('formFields.type') }),
  ...
  {
    accessorKey: 'value',
    header: ({ column }) => h(DataTableColumnHeader, { column, title: t('formFields.value') }),

    cell: ({ row }) => {
      return h('div', { class: 'flex space-x-2' }, [
        h('span', { class: 'max-w-[500px] truncate ' }, row.getValue('value')),
      ])
    },
    enableSorting: false,
  },
  ...
  // },
  {
    ...
    header: ({ column }) => h(DataTableColumnHeader, { column, title: t('formFields.required') }),
  ...
  // {
  //   accessorKey: 'options',
  //   header: ({ column }) => h(DataTableColumnHeader, { column, title: t('formFields.options') }),
  ...
  // {
  //   accessorKey: 'status',
  //   header: ({ column }) => h(DataTableColumnHeader, { column, title: t('formFields.status') }),
